...
if [[ "$SCRIPT_DIR" != *"/time_series_augmentation_mix"* ]]; then
    echo "Error: The script directory does not contain '/time_series_augmentation_mix'. Using the default directory."
...
aug_tech=('--original' '--jitter' '--scaling' '--permutation' '--randompermutation' '--magwarp' '--timewarp' '--windowslice' '--windowwarp' '--rotation' '--spawner' '--dtwwarp' '--shapedtwwarp' '--wdba' '--discdtw' '--discsdtw' '--extra_tag')
