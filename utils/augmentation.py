...
def permutation(x, max_segments=5, seg_mode="equal"):
    ...
    for i, pat in enumerate(x):
        if num_segs[i] > 1:
            if seg_mode == "random":
                split_points = np.random.choice(x.shape[1] - 1, num_segs[i] - 1, replace=False)
                split_points.sort()
                splits = np.split(orig_steps, split_points)
            else:
                splits = np.array_split(orig_steps, num_segs[i])

            # Ensure splits have the same length
            ...
            equal_length_splits = [np.pad(split, (0, max_len - len(split)), 'edge') for split in splits]

            # Permute and concatenate
            warp = np.concatenate(np.random.permutation(equal_length_splits)).ravel()[:x.shape[1]]
            ret[i] = pat[warp]
        else:
            ret[i] = pat
